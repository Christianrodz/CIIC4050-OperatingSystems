...
int main (void)
{
    ...
    */
    ...
    word_list = (char**) malloc(sizeof(char*) * nlines);

    // READ WORDS AND POINT EACH ELEMENT OF WORD_LIST TO A WORD
    for (int i = 0; i < nlines; i++) {
        //TODO: INSERT YOUR CODE TO FILL word_list WITH THE STD INPUTS 
        word_list[i] = word; //Give it space to recieve word
        word = (char *) malloc(sizeof(char) * MAX_CHARS); //Reinitialize for another word
        scanf("%s",word_list[i]); //Add string to word list
    }
